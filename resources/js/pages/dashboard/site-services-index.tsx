...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Our Services', href: '/dashboard/site-services' },
];
...
function ServiceRowForm({ service }: { service: ServiceRow }) {
    ...
    return (
        <form
            ...
            onSubmit={(e) => {
                e.preventDefault();
                form.post(`/dashboard/site-services/${service.id}`, {
                    preserveScroll: true,
                    forceFormData: true,
                    onSuccess: () => {
        ...
        >
            <div className="grid gap-3 md:grid-cols-2">
                <div className="flex flex-col gap-2 md:col-span-2">
                    <div className="flex flex-wrap items-start gap-3">
                        ...
                        <label className="text-sm">
                            <span className="mb-1 block text-muted-foreground">Card image</span>
                            <input
                                ref={fileRef}
                                type="file"
                ...
                <textarea
                    ...
                    onChange={(e) => form.setData('desc_ar', e.target.value)}
                ...
                <textarea
                    ...
                    onChange={(e) => form.setData('desc_en', e.target.value)}
            ...
            <label className="mt-3 inline-flex items-center gap-2 text-sm">
                ...
                />
                Active (shown on home & services page)
            </label>
            <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <button
                    ...
                    onClick={() =>
                        deleteForm.delete(`/dashboard/site-services/${service.id}`, { preserveScroll: true })
                    }
...
export default function SiteServicesIndex({ services }: PageProps) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6 rounded-xl p-4">
                ...
                <p className="text-sm text-muted-foreground">
                    These cards appear in the <strong>Our Services</strong> section on the home page and on the{' '}
                    <code className="rounded bg-muted px-1">/services</code> page. Drag order is controlled by{' '}
                    <strong>Sort order</strong> (lower first).
                </p>
                ...
                <form
                    ...
                    onSubmit={(e) => {
                        e.preventDefault();
                        createForm.post('/dashboard/site-services', {
                            preserveScroll: true,
                            forceFormData: true,
                            onSuccess: () => {
                                createForm.reset();
                ...
                >
                    <h2 className="text-lg font-semibold">Add service card</h2>
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                        ...
                        />
                        <textarea
                            value={createForm.data.desc_ar}
                            onChange={(e) => createForm.setData('desc_ar', e.target.value)}
                            placeholder="Arabic description"
                            className="min-h-20 rounded-md border border-input bg-background px-3 py-2 text-sm md:col-span-2"
                        />
                        <textarea
                            value={createForm.data.desc_en}
                            onChange={(e) => createForm.setData('desc_en', e.target.value)}
                            placeholder="English description"
                            className="min-h-20 rounded-md border border-input bg-background px-3 py-2 text-sm md:col-span-2"
                            dir="ltr"
                        />
                ...
                <div className="space-y-3">
                    {services.map((service) => (
                        <ServiceRowForm key={`${service.id}-${service.updated_at}`} service={service} />
                    ))}
                </div>
            </div>
        </AppLayout>
